...
export const NewJobLineItemFormSchema = z
  .object({
    type: z.nativeEnum(JOB_LINE_ITEM_TYPE, { message: 'Required' }),
    ...
    totalProductCostPrice: z.coerce
      .number()
      .min(0, { message: 'Cannot be less than 0' }),
    totalTruckCostPrice: z.coerce
      .number()
      .min(0, { message: 'Cannot be less than 0' }),
    totalProductSellPrice: z.coerce
      .number()
      .min(0, { message: 'Cannot be less than 0' }),
    totalTruckSellPrice: z.coerce
      .number()
      .min(0, { message: 'Cannot be less than 0' }),
  ...
  .superRefine((values, ctx) => {
    ...
    if (values.type === JOB_LINE_ITEM_TYPE.COLLECTION) return;
    ...
    if (
      (values.truckSellUom === 'HOURLY' ||
        values.truckSellUom === 'LOAD' ||
        values.truckSellUom === 'KM') &&
      !values.truckSellQty
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (
      (values.truckCostUom === 'HOURLY' ||
        values.truckCostUom === 'LOAD' ||
        values.truckCostUom === 'KM') &&
      !values.truckCostQty
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
