...
import { JobAttachmentTableActions } from './job-attachment-table-actions';

export const getJobAttachmentColumns = (
  jobId: number,
): ColumnDef<JobAttachmentDTO>[] => [
  {
    id: 'fileName',
        ...
        <Tooltip delayDuration={300}>
          <TooltipTrigger asChild>
            <button
              ...
              onClick={() => void downloadJobAttachment(jobId, attachment)}
            >
              {value}
            </button>
          </TooltipTrigger>
  ...
  },
  {
    id: 'category',
    accessorFn: (row) =>
      JOB_ATTACHMENT_CATEGORY_LABELS[row.category] ?? row.category,
    header: ({ column }) => {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const attachment = row.original;
      return (
        <JobAttachmentTableActions jobId={jobId} attachment={attachment} />
      );
    },
  },
];
