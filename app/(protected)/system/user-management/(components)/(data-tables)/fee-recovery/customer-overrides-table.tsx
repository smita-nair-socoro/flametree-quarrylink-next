...
export function CustomerOverridesTable({
  ...
  const filtered = useMemo(() => {
    return MOCK_CUSTOMER_OVERRIDES.filter((row) => {
      ...
      const matchesRule = ruleFilter === 'all' || row.rule === ruleFilter;
      ...
      <CardContent className="p-6 space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Customers on global default follow the setting above automatically. Turn on Custom to
            set an exception. Changes apply to uninvoiced, unlocked dockets only.
          </p>
        </div>
                ...
                pageRows.map((row) => {
                  ...
                  return (
                    <React.Fragment key={row.id}>
                      <TableRow className={cn('bg-white hover:bg-gray-50', isOn && 'border-b-0')}>
                        <TableCell className="pl-4 font-medium">{row.customer}</TableCell>
                        ...
                        <TableCell className="text-right">{formatFee(row.feePerDocket)}</TableCell>

                        <TableCell className="text-right">{formatPastMonth(row.pastMonth)}</TableCell>
                        ...
                        <TableRow className="bg-gray-50 hover:bg-gray-50">
                          <TableCell colSpan={6} className="pl-4 py-3">
                            <div className="flex items-end gap-3">
                              <div className="space-y-1.5">
                                <Label className="text-xs text-muted-foreground">Override rule</Label>
                                <Select
                                  value={form.overrideRule}
                                  onValueChange={(val) =>
                                    handleFormChange(row.id, 'overrideRule', val)
              ...
              <div className="mb-4 flex h-5 items-center space-x-2 sm:mb-0">
                <p className="whitespace-nowrap text-sm font-medium text-muted-foreground">
                  ...
                  <span className="text-accent-foreground ml-2">
                    {formatNumberThousandSeparatorWithoutDecimal(filtered.length)}
                  </span>
                </p>
