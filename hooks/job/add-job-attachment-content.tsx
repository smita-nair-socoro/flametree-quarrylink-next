...
} from '@/app/(protected)/customer-operations/jobs/(components)/forms/schemas/job-attachment-form-schema';
...
export function AddJobAttachmentDescription() {
  return (
...
interface AddJobAttachmentContentProps {
  category: JOB_ATTACHMENT_CATEGORY | '';
  onCategoryChange: (value: JOB_ATTACHMENT_CATEGORY) => void;
  fileName: string;
...
export function AddJobAttachmentContent({
  category,
  onCategoryChange,
  fileName,
  onFileNameChange,
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (nextFile && !fileName.trim()) {
      onFileNameChange(displayNameFromFileName(nextFile.name));
    }
  ...
  return (
    <div className="flex min-w-0 flex-col gap-4 overflow-hidden">
      <div className="flex flex-col gap-2">
        <Label htmlFor="job-attachment-category">Category*</Label>
        <Select
          value={category || undefined}
          onValueChange={(value) =>
            onCategoryChange(value as JOB_ATTACHMENT_CATEGORY)
          }
        >
          <SelectTrigger
            id="job-attachment-category"
            className="w-full"
          ...
          <SelectContent>
            {JOB_ATTACHMENT_CATEGORY_OPTIONS.map((option) => (
      ...
      <div className="flex flex-col gap-2">
        <Label htmlFor="job-attachment-file-name">File Name*</Label>
        <Input
          id="job-attachment-file-name"
