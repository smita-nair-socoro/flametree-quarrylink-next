...
import {
  useProductsForForm,
  productListItemFromQuotationLineItem,
...
export function useLineItemFormState({
  ...
  const getFormValuesFromLineItem = React.useCallback((): FormValues => {
    return {
      ...
      densityTonnagePerM3: isEditing
        ? (selectedLineItem?.densityTonnagePerM3 ?? 0)
        : 0,
  ...
  const linkedProductFromLineItem = React.useMemo(
    () =>
      selectedLineItem
        ? productListItemFromQuotationLineItem(selectedLineItem)
        : null,
    [selectedLineItem],
  );
  ...
  const {
    ...
    allowProductChangeWhileEditing: isEditing && !isReadOnly,
