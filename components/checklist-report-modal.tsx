    ...
    <>
      {isDesktop ? (
        ...
        <Drawer open={open} onOpenChange={onOpenChange}>
          <DrawerContent
            className="mt-0 flex h-[92dvh] max-h-[92dvh] w-full flex-col gap-0 overflow-hidden p-0"
          >
            <div className="shrink-0 border-b px-4 pb-4 pt-2">
              <DrawerTitle className="text-2xl font-bold">{title}</DrawerTitle>
            </div>
            <div className="flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto overscroll-contain px-4 py-4 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
              {body}
            </div>
          </DrawerContent>
        </Drawer>
